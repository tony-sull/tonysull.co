...
const { articleSchema, bookmarkSchema, noteSchema, personSchema, photoSchema } =
  createSchemas()
...
export const collections = {
  ...
  articles: defineCollection({
    schema: ({ image }) =>
      articleSchema.extend({
        ...
        featured: image()
          .describe(
            'primary photo for an article suitable for use in a link preview',
          )
          .optional(),
      }),
  }),
  ...
  personas: defineCollection({
    ...
    schema: ({ image }) =>
      personSchema.required({ nickname: true }).extend({
        logo: image().describe(
  ...
  photos: defineCollection({
    schema: ({ image }) =>
      photoSchema.extend({
        photo: image().describe('src URL for the original image file'),
      }),
  }),
}
