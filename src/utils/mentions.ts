...
async function fetchWebmentions(since: number, perPage = 1000) {
  ...
  if (response.ok) {
    ...
    console.log(
      `>>> ${feed.children.length} new webmentions fetch from ${API}`
    );
    return feed;
  }
...
let _mentionsPromise: Promise<{ children: WebMention[] }>;
export async function getAllMentions() {
  _mentionsPromise = _mentionsPromise || _getAllMentions();
  return await _mentionsPromise;
}

function compareUrls(a: string, b: string) {
  return a.replace(/(\/#|\/|#)$/, "") === b.replace(/(\/#|\/|#)$/, "");
}

export function isForUrl(url: string) {
  return function (webmention: WebMention) {
